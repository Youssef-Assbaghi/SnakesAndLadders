...
public final class Board {
    ...
    private void makeSnakes(int[][] snakes) {
		for (int i=0; i<snakes.length ; i++) {
			...
			snakeSquare.setSquareRole(new SpecialSquare(snakeSquare,transport,0));
		}
    }
		...
		private void makeDeaths(int[] deaths){
			for (int i=0; i<deaths.length; i++) {
				...
				int fromPosition=deaths[i];
				Square ladderSquare = squares.get(fromPosition);
				ladderSquare.setSquareRole(new SpecialSquare(ladderSquare,0,2));
			}
		}
    private void makeLadders(int[][] ladders) {
		for (int i=0; i<ladders.length; i++) {
			...
			Square ladderSquare = squares.get(fromPosition);
			ladderSquare.setSquareRole(new SpecialSquare(ladderSquare,transport,1));
		}
    }    
}
